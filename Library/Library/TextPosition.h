...
Expandable<NoteTab, 0> tabs;      // NoteTab positions/Left/Right
...
public:
  ...
  void setTab( int charPos, bool right);     // Insert an character position for the next
                                             // tab position
  NoteTab tab();                             // return position of next tab and sets cursorPos
  NoteTab findNextTab();
  void centerPos(int width) {pos((rightEdge - leftEdge - width) / 2 + leftEdge);}
  void rightPos( int width) {pos(rightEdge - width);}
  void rightTabPos(NoteTab& tab, int width) {pos(tab.pos - width);}
...
#if 0
struct NoteTab {
int  pos;                         // NoteTab position in pixels
